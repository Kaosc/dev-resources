## Learn & Practice

- [Developer Roadmaps - roadmap.sh](https://roadmap.sh/)
- [Scriba](https://scrimba.com/)
- [Codédex](https://www.codedex.io/)
- [CodeCrafters](https://app.codecrafters.io)
- [SQLBolt - Learn SQL](https://sqlbolt.com/lesson/)
- [Learn Git Branching](https://learngitbranching.js.org/)
- [Hacks Planining - Pick a Vulnerability to Learn About](https://www.hacksplaining.com/)
- [The Algorithms](https://the-algorithms.com/)
- [SQL Playground](https://sqlplayground.app/)
- [Edx](https://home.edx.org/)

## UI
- [Ant Design - The world's second most popular React UI framework](https://ant.design/)
- [Animate.css | A cross-browser library of CSS animations.](https://animate.style/)
- [shadcn/ui](https://ui.shadcn.com/)
- [React Aria](https://react-spectrum.adobe.com/react-aria/index.html)
- [TW Elements - 500+ free Tailwind CSS components](https://tw-elements.com/)
- [Flowbite - Build websites even faster with components on top of Tailwind CSS](https://flowbite.com/)
- [cubic-bezier ✿ cubic-bezier.com](https://cubic-bezier.com/)
- [MUI: The React component library you always wanted](https://mui.com/)
- [Slider](https://ui.shadcn.com/)
- [NativeWind](https://www.nativewind.dev/)
- [Remotion | Make videos programmatically in React](https://www.remotion.dev/)
- [Tremor • Components](https://www.tremor.so/components)
- [Rive - Build interactive animations that run anywhere](https://rive.app/)
- [Ionic Framework - The Cross-Platform App Development Leader](https://ionicframework.com/)
- [Recipes - Modern JS Code Snippets](https://buildui.com/recipes)
- [Framer](https://www.framer.com/motion/)
- [Buttons Generator](https://markodenic.com/tools/buttons-generator/)
- [Next.js Templates](https://nextjstemplates.com/templates?type=free)

## Softwares
- [Radon IDE – An IDE for React Native](https://ide.swmansion.com/)
- [Defender Control v2.1](https://www.sordum.org/9480/defender-control-v2-1/)

## CheetSheet
- [OverAPI.com | Collecting all the cheat sheets](https://overapi.com/)
- [Cheatsheets | Codecademy](https://www.codecademy.com/resources/cheatsheets/all)
- [Easing Functions Cheat Sheet](https://easings.net/)

## Dev Docs
- [Devhints - Dev Documentation](https://devhints.io/)
- [TutorialKit](https://tutorialkit.dev/)

## Web Tool 
- [GRID: A simple visual cheatsheet for CSS Grid Layout](https://grid.malven.co/)
- [FLEX: A simple visual cheatsheet for flexbox](https://flexbox.malven.co/)
- [CopyChar – Copy special characters to your clipboard](https://copychar.cc)
- [Color Designer - Simple Color Palette Generator](https://colordesigner.io/)

## Web Resources
- [Products, Tools and Resources for Developers & Designers | Resource.fyi](https://resource.fyi/)
- [Open Source Alternatives To Proprietary Software](https://www.opensourcealternative.to/)
- [freemediaheckyeah](https://fmhy.net/)

## Libraries & Frameworks
- [useHooks – The React Hooks Library](https://usehooks.com/)
- [Free Live Chat Software](https://www.tawk.to/)

## Image Hosting
- [Postimages — free image hosting / image upload](https://postimages.org/)

## CV
- [Reactive Resume](https://rxresu.me/)
- [Dashboard – FlowCV](https://app.flowcv.com/dashboard)

## Placeholder
- [Placeholder SVG file Urls](https://dev.w3.org/SVG/tools/svgweb/samples/svg-files/)